...
// a_min for
void define_viewport(float yaw, float pitch, int *tiles, int *num_tiles)
{
    *num_tiles = 0;
    ...
    if (x_min < 0) // x_min < 0
    {
        ...
        for (COUNT row = 0; row < NO_TILE_ROWS; row++)
        {
            for (COUNT cols = 0; cols < NO_TILE_COLS; cols++)
            {
                ...
                if (intersects(tile_x_min, tile_x_max, part1_x_min, part1_x_max) &&
                    intersects(tile_y_min, tile_y_max, y_min, y_max))
                {
                    int tile_id = row * NO_TILE_COLS + cols;
                    tiles[(*num_tiles)++] = tile_id;
                }
                else if (intersects(tile_x_min, tile_x_max, part2_x_min, part2_x_max) &&
                         intersects(tile_y_min, tile_y_max, y_min, y_max))
                {
                    int tile_id = row * NO_TILE_COLS+ cols;
                    tiles[(*num_tiles)++] = tile_id;
