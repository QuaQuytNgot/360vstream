                         ...
                         COUNT version_count)
{
    ...
    {
        self->curr_seg[i] = malloc(version_count * sizeof(buffer_t));
        ...
        for (COUNT s = 0; s < version_count; ++s)
        {
    ...
    for (COUNT i = 0; i < tile_count; i++)
    {
        self->dls[i] = malloc(seg_count * sizeof(bw_t));
...
// init cac tham so trong struct, init mang curr_seg de luu file .bin,
// init mang dls de luu past_bw
                         ...
                         int *chosen_versions)
{
    ...
    // First, download viewport tiles with their chosen versions
    for (int i = 0; i < num_vp_tiles; i++)
    {
        int tile_id = vp_tiles[i];
        int version = chosen_versions[i];

        if (tile_id >= self->tile_count || version >= self->version_count)
        {
            continue;
        }

        snprintf(url, sizeof(url), "%s/beach_%d/erp_8x6/tile_yuv/tile_%d_%d_480x360_QP%d.bin",
                 self->ser_addr,
                 chunk_id - 1,
                 tile_id / NO_OF_COLS,
                 tile_id % NO_OF_COLS,
    ...
    for (COUNT tile_id = 0; tile_id < self->tile_count; tile_id++)
    {
        ...
        if (!is_viewport_tile)
        {
            snprintf(url, sizeof(url), "%s/beach_%d/erp_8x6/tile_yuv/tile_%d_%d_480x360_QP38.bin",
                 self->ser_addr,
                 chunk_id - 1,
                 tile_id / NO_OF_COLS,
                 tile_id % NO_OF_ROWS);
...
{
    if (self == NULL || real_bw == NULL)
    {
        return RET_FAIL;
    }

    // Create a temporary array to hold all bandwidth measurements
    bw_t *dls_arr = malloc(self->tile_count * sizeof(bw_t));
    if (dls_arr == NULL_POINTER)
    {
        return RET_FAIL;
    }

    // Collect all valid bandwidth measurements
    COUNT valid_count = 0;
    for (COUNT i = 0; i < self->tile_count; i++)
    {
        // We'll use the bandwidth measurement from the first segment as representative
        bw_t latest_bw = 0.0;
        for (COUNT j = 0; j < self->seg_count; j++)
        {
            if (self->dls[i][j] > latest_bw)
            {
                latest_bw = self->dls[i][j];
            }
        }

        if (latest_bw > 0.0)
        {
            dls_arr[valid_count++] = latest_bw;
        }
    }

    // Use harmonic mean to calculate bandwidth
    bw_estimator_t bw_est;
    bw_estimator_init(&bw_est, BW_ESTIMATOR_HARMONIC);

    // Post the collected bandwidth data
    if (valid_count > 0)
    {
        bw_est.post(&bw_est, dls_arr, valid_count);

        // Get the estimated bandwidth
        bw_est.get(&bw_est, real_bw);
    }
    else
    {
        *real_bw = BW_DEFAULT;
