...
int main(int argc, char **argv)
{
    ...
    for (COUNT chunk_id = 0; chunk_id < handler.seg_count; chunk_id++)
    {
        ...
        tse.select_viewport(current_yaw,
                            current_pitch,
                            vp_tiles,
                            &num_tiles);
        // them ham dynamic tiling stratgy to change vp_tiles and num_tiles here
        // dynamic_tiling_strategy(&vp_tiles, &num_tiles, buffer_level, current_bw_estimate);
        ...
        // should declare a weighted parameter here to allocate current_bw_estimate
        ...
        for (int i = 0; i < num_tiles; i++)
        {
            ...
            chosen_versions[i] = ase.choose_bitrate(bwes.dls_es,
                                                    3,
                                                    buffer_level,
                                                    ase.last_quality_default);
        ...
        RET post_result = handler.post(&handler, chunk_id, vp_tiles, num_tiles, chosen_versions);
        ...
        for (int i = 0; i < num_tiles; i++)
        {
            ...
            bw_t speed = handler.dls[tile_id][chunk_id % handler.seg_count];
        ...
        printf("Chunk %d: Buffer level: %.2f, BW estimate: %.2f Mbps\n",
               chunk_id, buffer_level, current_bw_estimate / 1000000.0);
